<!DOCTYPE html>
<!--[if lt IE 7 ]> <html lang="en" class="ie6 ie6down ie7down ie8down ie9down"> <![endif]-->
<!--[if IE 7 ]>    <html lang="en" class="ie7 ie7down ie8down ie9down"> <![endif]-->
<!--[if IE 8 ]>    <html lang="en" class="ie8 ie8down ie9down"> <![endif]-->
<!--[if IE 9 ]>    <html lang="en" class="ie9 ie9down"> <![endif]-->
<!--[if (gt IE 9)|!(IE)]><!--> <html lang="en"> <!--<![endif]-->
<head>
    
    <title></title>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <meta name="viewport" content="width=device-width, minimum-scale=1.0, maximum-scale=1.0" />
    <meta name="description" content="">
    
    <link href="../assets/css/vendor.min.css" rel="stylesheet" type="text/css" />
    <link href="../assets/css/screen.min.css" rel="stylesheet" type="text/css" />
    
    <script type="text/javascript" src="../assets/js/dist/vendor.min.js"></script>
    <script type="text/javascript" src="../assets/js/dist/core.min.js"></script>
    
</head>
<body>

    <header id="header">
        <div class="container">
            <div class="row top">
                <div class="col-xs-12">
                    <ul class="list-unstyled list-inline">
                        <li><a href="#">Log in</a></li>
                        <li><a href="#">Register</a></li>
                    </ul>
                </div> <!-- left -->
                <div class="col-xs-12 text-right">
                    <ul class="list-unstyled list-inline">
                        <li><a href="#">Basket - <b>&pound;100</b></a></li>
                    </ul>
                </div> <!-- right -->
            </div> <!-- row -->

            <div class="row middle">
                <div class="col-xs-6">
                    <form action="" class="form form-search collapse navbar-collapse">
                        <input type="text" name="q" placeholder="Search for stuff..."  class="pull-right clearfix"/>
                    </form>
                    <div class="expand-menu">
                        <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#navbar-collapse">
                            <i class="sprite menu"></i>
                        </button> 
                    </div>
                </div> <!-- right -->
                <div class="logo col-xs-12 col-xs-offset-6">
                    <a href="/"><img src="../assets/images/logo.png" alt="Website Name" /></a>
                </div> <!-- logo -->
            </div> <!-- middle -->
            <nav class="nav collapse navbar-collapse navbar-toggle text-center" id="navbar-collapse" role="navigation">
                <ul class="list-unstyled">
                    <li class="on"><a href="#">Home</a></li>
                    <li><a href="#">About</a></li>
                    <li><a href="#">Products</a></li>
                    <li><a href="#">Other</a></li>
                    <li><a href="#">Contact</a></li>
                </ul>
            </nav>
        </div> <!-- conatiner -->
    </header> <!-- header -->