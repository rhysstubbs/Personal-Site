...
$cache_dir = "$dir/assets/css/cache";
$import_dirs = array("$dir/assets/css");

$scss = new scssc();
$scss->setImportPaths($import_dirs);

$server = new scss_server($dir, $cache_dir, $scss);
$server->serve();
...
class scssc {
	...
	static protected $numberPrecision = 4;
		...
		case "assign":
			...
			if ($name[0] == "var") {
				...
				if ($isDefault) {
					...
					$shouldSet = $existingValue == self::$null;
				}
				
				//print_r($name[1]." at ".print_r($value, true)." ".($isDefault ? "is" : "is not")." default and ".($shouldSet ? "should be set" : "should not be set").".");

				if (!$isDefault || $shouldSet) {
					//print(".. so is being set.\n\n");
